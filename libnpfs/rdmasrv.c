...
extern Nptrans *np_rdmatrans_create(struct rdma_cm_id *cmid, int q_depth, int msize, int srv);
...
np_rdmasrv_create(int nwthreads, int *port)
{
	...
	rdma = calloc(1, sizeof(struct Rdmasrv));
	if (!rdma)
		return NULL;
...
rdmasrv_listenproc(void *a)
{
	...
	while (!rdma->shutdown) {
		...
		case RDMA_CM_EVENT_CONNECT_REQUEST:
			...
			trans = np_rdmatrans_create(cmid, srv->nwthread, srv->msize, 1);
			if (trans) {
